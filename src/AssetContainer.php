...
class AssetContainer
{
     ...
     * @return AssetContainer
     */
    public function add($name, $source = null, $dependencies = [], $attributes = [])
    {
        if (!is_array($name)) {
            if (!isset($source)) {
                throw new \ErrorException("Missing argument 2 for Fcl\Theme\AssetContainer::add()", 1);
            }
        ...
        foreach ($name as $array) {
            if (count($array) < 2) {
                throw new \ErrorException("Missing value 2 of the array for Fcl\Theme\AssetContainer::add()", 1);
            }
